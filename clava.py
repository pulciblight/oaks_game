...
def alternate_scenario(play, checkpoint, choices, player_sex, player_lives, player_rep, player_disc, player_money):
    ...
    if checkpoint == 'Попрошу одногруппницу скинуть свое' and player_rep < -3:
        ...
        play['options'] = ['Напишу самостоятельно', 'Закинуть промпт в chatGPT',
                           'Не сдавать работу']
        del play['conseq']
    if checkpoint == 'Продолжить день' and ('Долг' or 'Долг в Вышке') in choices:
        if 'Долг' in choices:
    ...
    if checkpoint == 'Пора готовиться ко сну' and ('Долг' or 'Долг в Вышке') in choices:
        if 'Долг' in choices:
            if player_sex == 'м':
        ...
        if 'Трижды должник' in choices:
            ...
            if player_sex == 'м':
                play['text'] = ('Ты прошёл игру на пять с плюсом! Ни одной дисциплинарки и почти идеальные репутация и здоровье. Ты написал заявку на переселение в Трилист в Одинцово и '
                    'уже черед неделю собирал чемоданы. Переезд был сложным, но теперь у тебя есть на 20 минут больше сна ежедневно. '
                ...
                play['text'] = (
                    ...
                    'уже черед неделю собирала чемоданы. Переезд был сложным, но теперь у тебя есть на 20 минут больше сна ежедневно. '
                    'Ты только посчитай, сколько это в год! Новые соседи, новая комната, почти новая жизнь. Тебе удалось покинуть Дубки, поздравляем!')
            play['options'] = ['Начать заново', 'Закончить игру']
